...
void DrawCursorsAndDistance(Cursor *cursors, int count, PSF_Font font, int fontSize, DragRect *centerRect) {
    ...
    if (count >= 2) {
        ...
        DrawPSFText(font, textPos.x, textPos.y, distanceText, spacing, LIGHTGRAY);
    }

    // Вивід значень курсорів у нижній частині екрану
    Vector2 curPosA = {10, GetScreenHeight() - 60};
    DrawPSFText(font, curPosA.x, curPosA.y, TextFormat("Значення курсора A: %i", cursors[0].value), spacing, cursors[0].color);
    Vector2 curPosB = {10, GetScreenHeight() - 30};
    DrawPSFText(font, curPosB.x, curPosB.y, TextFormat("Значення курсора B: %i", cursors[1].value), spacing, cursors[1].color);
}
