 ...
 */
void Gui_CheckBox(Rectangle bounds, bool *checked, const char *textTop, const char *textRight, Color color)
{
    ...
    Color textColor = GetContrastingTextColor(boxColor);  // Колір тексту для читабельності
    ...
    if(textRight != NULL && textRight[0] != '\0') {
        ...
        float textRightWidth = maxLineWidthCharsRight * (font32.width + localSpacing) - localSpacing + 2 * padding;
        float textRightHeight = lineCountRight * font32.height + (lineCountRight - 1) * LineSpacing + 2 * padding;
        ...
        DrawPSFText(font32, textRightPos.x, textRightPos.y, textRight, localSpacing, textColor);
    }
