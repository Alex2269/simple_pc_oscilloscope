    ...
    *value = changed;
    ...
    if (mouseOver && textTop && strlen(textTop) > 0) {
        ...
        for (int i = 0; i < lineCount; i++) {
            // float lineWidth = (float)font32.width * strlen(lines[i]) + (strlen(lines[i]) - 1) * spacing + padding;

            int charCount = utf8_strlen(lines[i]);
            float lineWidth = charCount * (font32.width + spacing) - spacing + padding;
            if (lineWidth > maxWidth) maxWidth = lineWidth;
        }

        // Висота одного рядка
        float lineHeight = (float)font32.height;
    ...
    // Vector2 textSize = { (float)font32.width * strlen(textRight),
    //                      (float)font32.height };

    int charCountRight = utf8_strlen(textRight);
    Vector2 textSize = { (float)charCountRight * (font32.width + spacing) - spacing,
                        (float)font32.height };
