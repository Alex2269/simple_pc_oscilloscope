...
int spacing = 2; // Відступ між символами, той самий, що передається у DrawPSFTextUTF8
    ...
    *value = changed;
    ...
    if (mouseOver && textTop && strlen(textTop) > 0) {
        ...
        for (int i = 0; i < lineCount; i++) {
            // float lineWidth = (float)psfFont.width * strlen(lines[i]) + (strlen(lines[i]) - 1) * spacing + padding;

            int charCount = utf8_strlen(lines[i]);
            float lineWidth = charCount * (psfFont.width + spacing) - spacing + padding;
            if (lineWidth > maxWidth) maxWidth = lineWidth;
        }

        // Висота одного рядка
        float lineHeight = (float)psfFont.height;
        ...
        for (int i = 0; i < lineCount; i++) {
            DrawPSFTextUTF8(psfFont,
                            tooltipRect.x + padding * 2,
                            tooltipRect.y + padding / 2 + i * (lineHeight + 2),
                            lines[i], 2, colorText);
        }
    }
    ...
    // Vector2 textSize = { (float)psfFont.width * strlen(textRight),
    //                      (float)psfFont.height };

    int charCountRight = utf8_strlen(textRight);
    Vector2 textSize = { (float)charCountRight * (psfFont.width + spacing) - spacing,
                        (float)psfFont.height };
    ...
    DrawPSFTextUTF8(psfFont,
                    center.x + radius + 10, center.y - radius / 2,
                    textRight, 2, textColor);
