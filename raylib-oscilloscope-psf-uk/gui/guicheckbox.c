...
extern int fontSize;       // Розмір шрифту (не використовується безпосередньо тут, але може бути корисним)
...
void Gui_CheckBox(Rectangle bounds, bool *checked, const char *textTop, const char *textRight, Color color)
{
    ...
    if (mouseOver && textTop && textTop[0] != '\0')
    {
        ...
        for (int i = 0; i < lineCountTop; i++) {
            ...
            float lineWidth = charCount * (psfFont.width + localSpacing) - localSpacing;
            if (lineWidth > maxWidthTop) maxWidthTop = lineWidth;
        }

        float lineHeightTop = (float)psfFont.height;
