...
int main(void) {
    ...
    while (1) {
        ...
        // DrawCursorsAndDistance(cursors, 2, Terminus12x6_font, Terminus12x6_font.height, &centerRect);

        // Vector2 startPos = {20, 20};
        // Vector2 endPos = {600, 320};
        // DrawLineEx(startPos, endPos, 5.0f, RED); // test line

        // Малювання поточних значень каналів в лівому верхнньому куті осцилоскопа
        uint32_t channel_colors[MAX_CHANNELS] = { YELLOW, GREEN, RED, SKYBLUE };
        for (int i = 0; i < MAX_CHANNELS; i++) {
            if (oscData.channels[i].active && oscData.channels[i].channel_history != NULL) {
                ...
                DrawTextWithAutoInvertedBackground(Terminus12x6_font, textPos.x, textPos.y,
                                                   TextFormat("Ch%d: %.0f", i+1, last_value),
                                                   spacing, scale, channel_colors[i],
        ...
        // DrawVerticalScale(1, Ch->scale_y, Ch->offset_y, scaleArea, Terminus12x6_font, WHITE);
